...
Run multiple MCMCs on simulated datasets and return DIC and the
effective number of parameters for each run.
...
`tar_jags_rep_dic()` returns list of target objects. See the "Target
...
- `x_y`: run JAGS on each dataset from `x_data`. Each dynamic branch
  returns a tidy data frame of DIC results for each batch of data.
...
- `x`: combine all the batches from `x_y` into a non-dynamic target.
  ...
  with all DIC info from all the branches of `x_y`.
