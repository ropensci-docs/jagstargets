...
`tar_jags_rep(name = x, jags_files = "y.jags")` returns a list of
[`targets::tar_target()`](https://docs.ropensci.org/targets/reference/tar_target.html)
objects:
...
- `x`: combine all the model-specific summary targets into a single data
  frame with columns to distinguish among the models. Suppressed if
  `combine` is `FALSE`.
