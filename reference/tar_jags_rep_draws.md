...
Run multiple MCMCs on simulated datasets and return posterior samples
...
`tar_jags_rep_draws()` returns list of target objects. See the "Target
...
- `x_y`: run JAGS on each dataset from `x_data`. Each dynamic branch
  returns a tidy data frame of draws for each batch of data.
...
- `x`: combine all the batches from `x_y` into a non-dynamic target.
  ...
  with all draws from all the branches of `x_y`.
